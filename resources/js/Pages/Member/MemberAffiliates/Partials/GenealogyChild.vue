...
const getDistributorDetail = async (id, level) => {
    ...
    try {
        let url = `/member/getDistributorDetail?id=` + id + `&level=${level}`;
...
<template>
    <div v-if="node" class="text-center">
        <div class="flex flex-col justify-center items-center">
            ...
            <div class="w-60 mt-5 rounded-lg dark:bg-gray-700 dark:hover:bg-gray-800 hover:cursor-pointer p-3 z-20"
                 @click="openDistributorDetailModal(node)">
                <div class="flex gap-2 items-center border-b border-dashed border-gray-600 pb-3">
                    ...
                    <div class="flex flex-col gap-1">
                        <div class="flex gap-2 items-center text-left text-gray-900 dark:text-white">
                            ...
                            <div
                                class="rounded-xl bg-warning-400 text-gray-800 font-normal text-xs px-2 text-center w-16">
                                Gen {{ node.level }}
                            </div>
                        </div>
        ...
        <ul v-if="isExpand" class="flex flex-row mt-4 justify-center">
            ...
            <li v-if="node.children && node.children.length > 0" v-for="child in node.children" :key="child.name"
                class="relative p-6">
                <div v-if="child.position === 'left'"
                     class="border-t-2 border-dashed absolute h-8 border-gray-600 top-0 left-1/2 right-0"></div>
                <div v-if="child.position === 'right'"
                     class="border-t-2 border-dashed absolute h-8 border-gray-600 top-0 left-0 right-1/2"></div>
                <GenealogyChild v-if="Object.keys(child).length > 0" :binaryTree="child"/>

                <!-- Add Distributor-->
                <div v-else>
                    <div v-if="binaryTree.children[0].position !== 'left'"
                         class="border-t-2 border-dashed absolute h-8 border-gray-600 top-0 left-1/2 right-0"></div>
                    <div v-if="binaryTree.children[1].position !== 'right'"
                         class="border-t-2 border-dashed absolute h-8 border-gray-600 top-0 left-1 right-1/2"></div>
                    <div v-if="binaryTree.children[0].position !== 'left'" class="relative flex justify-center">
                        ...
                        <div
                            class="flex items-center justify-center rounded-full bg-success-500 grow-0 shrink-0 w-8 h-8">
    ...
    <Modal :show="distributorDetailModal" title="Distributor Detail" @close="closeDetailModal">
        ...
        <div v-if="distributorDetail && !isLoading">
            <div
                class="flex items-center p-5 rounded-xl bg-gray-300 dark:bg-gray-700 gap-2 self-stretch w-full mt-3 mb-8">
                <img
                    :src="distributorDetail.profile_photo ? distributorDetail.profile_photo : 'https://img.freepik.com/free-icon/user_318-159711.jpg'"
                    class="w-8 h-8 rounded-full" alt="">
                <div class="flex flex-col items-start gap-1">
                    <div class="flex items-center gap-2">
                        <div class="text-gray-700 dark:text-white text-sm font-semibold">
            ...
            <div class="flex flex-col items-start gap-3 self-stretch">
                <div class="flex items-center gap-2 self-stretch">
                    ...
                    <div class="flex items-center gap-2">
                        <img
                            :src="distributorDetail.sponsor_profile_photo ? distributorDetail.sponsor_profile_photo : 'https://img.freepik.com/free-icon/user_318-159711.jpg'"
                            class="w-8 h-8 rounded-full" alt="">
                ...
                <div class="flex items-center gap-2 self-stretch">
                    ...
                    <div class="flex items-center gap-2">
                        <img
                            :src="distributorDetail.upline_profile_photo ? distributorDetail.upline_profile_photo : 'https://img.freepik.com/free-icon/user_318-159711.jpg'"
                            class="w-8 h-8 rounded-full" alt="">
                        <div class="text-base text-gray-800 dark:text-white">
