...
<template>
    <div class="relative overflow-x-auto sm:rounded-lg">
        ...
        <div v-else class="overflow-x-auto">
            <table class="w-[650px] md:w-full text-sm text-left text-gray-500 dark:text-gray-400 mt-5">
                ...
                <tr>
                    <th scope="col" class="pl-5 px-3 py-2">
                        Date
                    </th>
                    <th scope="col" class="px-3 py-2">
                        Name
                    </th>
                    <th scope="col" class="pl-10 px-3 py-2">
                        Type
                    </th>
                    <th scope="col" class="px-3 py-2">
                        Transaction ID
                ...
                >
                    ...
                    <td v-if="walletAndAsset.status == 'Success'" class="px-3 py-2 text-center">
    ...
    <Modal :show="walletsAndAssetsHistoryModal" title="Wallet Adjustment Details" @close="closeModal">
        <div class="grid grid-cols-3 items-center gap-2">
            ...
            <span class="col-span-2 text-black dark:text-white px-3 py-2">{{ walletsAndAssetsDetail.user.name }}</span>
        </div>
        <div class="grid grid-cols-3 items-center gap-2">
            <span class="col-span-1 text-sm font-semibold dark:text-gray-400">Email</span>
            <span class="col-span-2 text-black dark:text-white px-3 py-2">{{ walletsAndAssetsDetail.user.email }}</span>
        </div>
        <div class="grid grid-cols-3 items-center gap-2">
            <span class="col-span-1 text-sm font-semibold dark:text-gray-400">Date & Time</span>
            <span class="col-span-2 text-black dark:text-white px-3 py-2">{{ formatDateTime(walletsAndAssetsDetail.created_at) }}</span>
        </div>
        <!-- <div class="grid grid-cols-3 items-center gap-2">
            <span class="col-span-1 text-sm font-semibold dark:text-gray-400">Before Adjustment</span>
            <div v-if="walletsAndAssetsDetail.transaction_type === 'WalletAdjustment'" class="col-span-2 text-black dark:text-white px-3 py-2">
                <span>$ </span>{{ formatAmount(walletsAndAssetsDetail.new_amount) }}
            </div>
            <div v-if="walletsAndAssetsDetail.transaction_type === 'AssetAdjustment'" class="col-span-2 text-black dark:text-white px-3 py-2">
        ...
        <div class="grid grid-cols-3 items-center gap-2">
            ...
            <span class="col-span-2 text-black dark:text-white px-3 py-2">{{ walletsAndAssetsDetail.remarks }}</span>
        </div>
    </Modal>
</template>