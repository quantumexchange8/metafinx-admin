...
const earning = ref({data: []});
const totalAmount = ref(0);
const currentPage = ref(1);
const refreshDividend = ref(props.refresh);
...
const getResults = async (page = 1, search = '', date = '') => {
    ...
    try {
        let url = `/report/getEarningPayoutDetails/AffiliateDividendEarnings/standard?page=${page}`;
        ...
        earning.value = response.data.results;
...
watch(() => props.exportStatus, (newVal) => {
    refreshDividend.value = newVal;
    if (newVal) {
        let url = `/report/getDividendEarningPayoutDetails?exportStatus=yes`;
...
<template>
    ...
    <div v-else class="overflow-x-auto">
        <table class="w-[650px] md:w-full text-sm text-left text-gray-500 dark:text-gray-400 mt-5 table-fixed">
            ...
            <tr
                v-for="dividend in earning.data"
            ...
            >
                <td class="px-3 py-2.5">
                    <div class="flex items-center gap-2">
                        <img :src="dividend.user?.profile_photo_url || 'https://img.freepik.com/free-icon/user_318-159711.jpg'" class="w-8 h-8 rounded-full" alt="">
                        <div class="flex flex-col">
                            <div>
                                {{ dividend.user ? dividend.user.name : 'N/A' }}
                            </div>
                            <div class="dark:text-gray-400">
                                {{ dividend.user ? dividend.user.email : 'N/A' }}
                            </div>
                        </div>
                    </div>
                </td>
